...
def get_capabilities_block(agent_config: dict) -> str:
    ...
    lines.append("")
    lines.append("**WICHTIG — konkretes Format der Aktions-Tags** (JSON im Tag, inline eingebettet in deiner Antwort):")
    lines.append("")
    lines.append("Word (.docx):")
    lines.append('`[CREATE_FILE:docx:{"title":"Dateiname","content":[{"type":"heading","level":1,"text":"Titel"},{"type":"paragraph","text":"Absatz"},{"type":"bullet","text":"Punkt"},{"type":"table","rows":[["Spalte1","Spalte2"],["Wert1","Wert2"]]}]}]`')
    lines.append("")
    lines.append("Excel (.xlsx):")
    lines.append('`[CREATE_FILE:xlsx:{"title":"Dateiname","sheets":[{"name":"Tabelle1","rows":[["Kopf1","Kopf2"],["Wert1","Wert2"]]}]}]`')
    lines.append("")
    lines.append("PDF:")
    lines.append('`[CREATE_FILE:pdf:{"title":"Dateiname","content":[{"type":"heading","level":1,"text":"Titel"},{"type":"paragraph","text":"Text"}]}]`')
    lines.append("")
    lines.append("PowerPoint (.pptx):")
    lines.append('`[CREATE_FILE:pptx:{"title":"Praesentation","slides":[{"type":"title","heading":"Untertitel"},{"type":"content","heading":"Folientitel","body":"Text","bullets":["Punkt 1","Punkt 2"]}]}]`')
    lines.append("")
    lines.append("E-Mail-Draft (oeffnet Apple Mail):")
    lines.append('`[CREATE_EMAIL:{"to":"empfaenger@example.com","cc":"","subject":"Betreff","body":"Text","from":"optional@absender.com"}]`')
    lines.append("")
    lines.append("E-Mail-Antwort (mit Threading via Message-ID aus der Original-Mail):")
    lines.append('`[CREATE_EMAIL_REPLY:{"message_id":"<original-id@domain.com>","to":"absender@example.com","cc":"","subject":"Re: Betreff","body":"Antwort","quote_original":true}]`')
    lines.append("")
    lines.append("WhatsApp (oeffnet WhatsApp mit vorgefuellter Nachricht):")
    lines.append('`[CREATE_WHATSAPP:{"to":"Vorname","message":"Nachrichtentext"}]`')
    lines.append("")
    lines.append("Slack (via API wenn Bot-Token gesetzt):")
    lines.append('`[CREATE_SLACK:{"channel":"#kanal-oder-@user","text":"Nachricht"}]`')
    lines.append("")
    lines.append("Regeln: (1) JSON muss gueltig sein, doppelte Anfuehrungszeichen, keine Trailing Commas. (2) Tag steht im Antwort-Text (kein Markdown-Fence drumherum). (3) Das Backend ersetzt den Tag durch einen Downloadlink und kopiert die Datei auch ins Agent-Memory. (4) Bei CREATE_EMAIL_REPLY die Message-ID aus der Original-Mail lesen (Header-Feld `Message-ID:`).")
    lines.append("")
    lines.append("### Uebersicht der Typen:")
