#!/usr/bin/env python3
"""Patch: Replace Email Reply Modal with chat-native email flow.
- Remove old modal HTML, CSS, JS
- Add /api/email-content backend route
- Add email card in chat + context passing to agent
- /create-email-reply now triggers inline email search in chat
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# PATCH 1: Replace old Modal CSS with Email-Card-in-Chat CSS
# ══════════════════════════════════════════════════════════════════════════════
old_css = """  /* Email Reply Modal */
...
new_css = """  /* Email Card in Chat */
  ...
  .email-card-icon { display:inline-block; margin-right:8px; font-size:16px; vertical-align:middle; }
  ...
  .email-search-results { margin:8px 0; }
  ...
  .email-search-card-hint { color:#666; font-size:12px; padding:8px 0; text-align:center; }
  """

if old_css not in content:
    print("FEHLER: Alte Modal CSS nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_css, new_css)
changes += 1
print(f"Patch {changes}: CSS ersetzt (Modal -> Email-Card-in-Chat)")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 2: Remove old Modal HTML
# ══════════════════════════════════════════════════════════════════════════════
old_modal_html = """<div id="email-reply-modal">
...
new_modal_html = """<div id="search-overlay">"""

if old_modal_html not in content:
    print("FEHLER: Altes Modal HTML nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_modal_html, new_modal_html)
changes += 1
print(f"Patch {changes}: Altes Modal HTML entfernt")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 3: Replace old Modal JS with new chat-native email flow JS
# ══════════════════════════════════════════════════════════════════════════════
old_js = """// ─── EMAIL REPLY MODAL ─────────────────────────────────────────────────────
...
new_js = """// ─── EMAIL CHAT FLOW (replaces old modal) ────────────────────────────────
let _emailContext = null;  // Stores {from_email, from_name, subject, message_id, cc} for next send
...
function _emailSearchInChat(query) {
  ...
  fetch('/api/email-search?agent=' + encodeURIComponent(agent) + '&q=' + encodeURIComponent(query))
    ...
    .then(results => {
      ...
      // Insert search results as clickable cards in chat
      var msgs = document.getElementById('messages');
      var div = document.createElement('div');
      div.className = 'msg assistant';
      var time = new Date().toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit'});
      var html = '<div class="bubble"><div class="email-search-results">';
      html += '<div style="font-size:12px;color:#6a8aca;margin-bottom:8px;font-weight:600;">\\u2709 ' + results.length + ' E-Mail(s) gefunden</div>';
      results.forEach(function(item, idx) {
        // Use data attributes to pass email data
...
function _showEmailCard(email) {
  ...
  div.className = 'msg assistant';
  var time = new Date().toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit'});
  var subj = email.subject || '(kein Betreff)';
  ...
  var html = '<div class="bubble"><div class="email-card">'
    ...
    + '<div class="email-card-subject">' + escHtml(subj) + '</div>'
    ...
    + '<button class="email-card-btn email-card-btn-reply" id="ec-reply-btn">Antworten</button>'
    + '<button class="email-card-btn email-card-btn-close" id="ec-close-btn">Schliessen</button>'
    + '</div>'
  ...
  // Reply button: set email context and focus input
  div.querySelector('#ec-reply-btn').onclick = function() {
    ...
    [email.to || '', email.cc || ''].forEach(function(field) {
      ...
      field.split(',').forEach(function(addr) {
        ...
        if (em && !ownAddrs.includes(em.toLowerCase().trim())) {
          ccParts.push(addr);
        }
      });
    });
    ...
    // Remove reply button highlight
    this.textContent = '\\u2713 Kontext gesetzt';
    this.style.background = '#2a5a2a';
    this.onclick = null;
  };
  // Close button: remove card
  div.querySelector('#ec-close-btn').onclick = function() { div.remove(); };
}
...
function selectSlashCmd(inputEl, cmd) {
  ...
  if (cmd === '/create-email-reply') {
    inputEl.value = '';
    hideSlashAutocomplete();
    // Show prompt for email search
    addStatusMsg('Tippe einen Suchbegriff (Name, E-Mail oder Betreff) und druecke Enter.');
    inputEl.focus();
    return;
  }"""

if old_js not in content:
    print("FEHLER: Alter Modal JS-Block nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_js, new_js)
changes += 1
print(f"Patch {changes}: JS ersetzt (Modal -> Chat-native Email Flow)")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 4: Intercept /reply-email search in sendMessage
# Add email search handling before regular chat send
# ══════════════════════════════════════════════════════════════════════════════
old_send_prefix = """  addMessage('user', text);
  scrollDown();
  doSendChat(text);
}"""

new_send_prefix = """  addMessage('user', text);
  scrollDown();
  // EMAIL_REPLY_SEARCH: if text looks like an email search (short, no question marks, not a sentence)
  // and was triggered right after /create-email-reply hint, search emails
  if (text.startsWith('/reply ') || text.startsWith('/reply-email ')) {
    var emailQuery = text.replace(/^\\/reply(-email)?\\s+/, '').trim();
    if (emailQuery.length >= 2) { _emailSearchInChat(emailQuery); return; }
  }
  doSendChat(text);
}"""

if old_send_prefix not in content:
    print("FEHLER: sendMessage Suffix nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_send_prefix, new_send_prefix, 1)
changes += 1
print(f"Patch {changes}: /reply Suchbefehl in sendMessage eingefuegt")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 5: Inject email context into doSendChat
# ══════════════════════════════════════════════════════════════════════════════
old_dosend = """async function doSendChat(text) {
  startTyping(text.substring(0,50));"""

new_dosend = """async function doSendChat(text) {
...
if old_dosend not in content:
    print("FEHLER: doSendChat nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_dosend, new_dosend)
changes += 1
print(f"Patch {changes}: Email-Kontext-Injection in doSendChat")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 6: Add /reply slash command to the commands list
# ══════════════════════════════════════════════════════════════════════════════
old_slash_reply = "  {cmd: '/create-email-reply', label: '/create-email-reply', desc: 'E-Mail Antwort erstellen', template: 'Antworte auf die E-Mail von [Absender] zum Thema [Betreff]: ', group: 'Kommunikation'},"

new_slash_reply = """  {cmd: '/create-email-reply', label: '/create-email-reply', desc: 'E-Mail Antwort erstellen', template: 'Antworte auf die E-Mail von [Absender] zum Thema [Betreff]: ', group: 'Kommunikation'},
  {cmd: '/reply', label: '/reply [suche]', desc: 'E-Mail suchen und im Chat oeffnen', template: '/reply ', group: 'Kommunikation'},"""

if old_slash_reply not in content:
    print("FEHLER: Slash command /create-email-reply nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_slash_reply, new_slash_reply)
changes += 1
print(f"Patch {changes}: /reply Slash-Command hinzugefuegt")

# ══════════════════════════════════════════════════════════════════════════════
# PATCH 7: Backend route /api/email-content
# Insert after /api/email-search route
# ══════════════════════════════════════════════════════════════════════════════
old_route_end = """    # Sort by date_ts descending
    results.sort(key=lambda r: r.get('date_ts', 0), reverse=True)
    return jsonify(results[:8])"""

new_route_end = """    # Sort by date_ts descending
    results.sort(key=lambda r: r.get('date_ts', 0), reverse=True)
    return jsonify(results[:8])
        ...
        try:
            for fname in os.listdir(sdir):
                ...
                try:
                    ...
                    # Match by message_id or by from+subject
...
if old_route_end not in content:
    print("FEHLER: email-search Route Ende nicht gefunden!", file=sys.stderr)
    sys.exit(1)
content = content.replace(old_route_end, new_route_end)
changes += 1
print(f"Patch {changes}: /api/email-content Backend-Route eingefuegt")

# Write result
with open(path, 'w') as f:
    f.write(content)

print(f"\nAlle {changes} Patches erfolgreich angewendet!")
