...
[2026-04-28] FIX+FEATURE: Klassifikator-Stems + Pattern-Analyzer Bootstrap-Delay (Roadmap-Polishing) | src/usage_logger.py + src/pattern_analyzer.py + src/web_server.py + tests/run_tests.py + frontend/src/components/FRONTEND_TODO_*.md (Lovable-Repo) | Drei Anpassungen nach Abschluss der April-Roadmap. (A) Klassifikator-Stems: User-Bug "Recherchiere Stripe vs Adyen" wurde als 'other' klassifiziert (statt 'research'), weil die alte substring-Logik 'recherche' nicht in 'recherchiere' findet (Buchstabe 'i' mittendrin). Recherche-Empfehlung (Snowball/Cistem vs Custom): Custom-Suffix-Stripper bei <=30 Keywords ausreichend, kein NLTK-Overhead. Implementation: _stem(word) mit Greedy-Suffix-Stripping (laengste zuerst), Min-Stem-Laenge 4, Umlaut-Normalisierung (ä->a, ö->o, ü->u, ß->ss), 36 Suffixe (iertest/iertet/ierten/ierte/ieren/iert/ierst/iere/ier/endes/enden/endem/ender/ende/ungen/ungs/ung/isch/ische/ischen/lich/liche/lichen/est/en/er/es/em/st/te/e/t/s/n). _LOANWORDS-Set fuer 30 Anglizismen (email/code/python/report/meeting/...) die nicht gestemmt werden — sonst wuerde 'email' zu 'emai'. Pre-Compile-Index (_STEM_INDEX) baut beim Modul-Load aus _KEYWORD_RULES: Single-Word-Keywords werden gestemmt + Dict-Lookup, Multi-Word-Keywords ('suche heraus', 'wer ist') laufen weiter ueber substring (Stage 1). Klassifikator: zuerst Multi-Word-Match (Stage 1), dann Token-basiert mit Stem-Dict-Lookup unter Beruecksichtigung der _KEYWORD_RULES-Reihenfolge (Stage 2). Side-Bug-Fix: ' repo' (mit fuehrendem Space) als coding-Keyword matchte falsch in ' report' — durch 'repository' ersetzt. Plus 'geschrieb' als zusaetzlicher writing-Stem fuer starke Verben ('geschrieben'). Test-Coverage von 16/20 -> 19/20 (Quartalsbericht-Kompositum bleibt edge case, kein Stemmer fixt das). (B) Pattern-Analyzer Bootstrap-Delay: User-Wunsch "in 10 Tagen ersten Versuch starten, danach selbstaendig mit Genehmigung". Heute ist 2026-04-28, +10 Tage = 2026-05-08. Neue Konstante BOOTSTRAP_FIRST_RUN_AFTER = 2026-05-08T22:00:00-03:00. Neuer Schedule-Predicate _bootstrap_aware_schedule: 1) wenn now < FIRST_RUN_AFTER -> nie ausloesen, 2) wenn now >= FIRST_RUN_AFTER UND last_run==None -> sofort ausloesen (initial Bootstrap), 3) danach Standard-weekly Sonntag 22:00 (>=6 Tage seit last_run). register_with_heartbeat() nutzt jetzt diesen Predicate statt heartbeat.weekly_at(6, 22, 0) direkt. Hinweis im Doc-String: "selbstaendig/proaktiv mit Genehmigung" ist genau das schon gebaute Pattern: Cron triggert analyze_logs() autonom -> Notifications via heartbeat.enqueue_notification mit yes/no/later-Buttons -> erst nach 'yes' wird ein Pattern als eigener Cron-Job aktiviert. Kein autonomer Side-Effect ohne explizite Genehmigung. (C) Neuer Endpoint GET /api/patterns/bootstrap_status liefert {first_run_after, now, in_bootstrap_phase, seconds_until_first_run} fuer Frontend (das FRONTEND_TODO_PATTERNS_VIEW.md zeigt das als Hinweis-Banner). (D) Frontend-Repo: drei FRONTEND_TODO_*.md gepusht (PATTERNS_VIEW, ORCHESTRATION, HEARTBEAT_PUSH) als Briefs an Lovable mit API-Specs + Akzeptanzkriterien. Live verifiziert: classify_task('Recherchiere Stripe vs Adyen') -> 'research' (vorher: 'other'); classify_task('Repository klonen') -> 'coding'; classify_task('Schreib mir einen Report') -> 'document' (vorher: 'coding' wegen ' repo'-Bug). bootstrap_status zeigt in_bootstrap_phase:true mit ~10.6 Tagen seconds_until_first_run. 35 neue Tests, Suite 1839/1841 (gleiche 2 pre-existing Pillow).

[2026-04-27] FEATURE: Orchestrator (Roadmap-Feature 6 April 2026) — Aufgaben-Decomposition + Best-Modell-pro-Skill-Routing | src/orchestrator.py (neu) + src/web_server.py + tests/run_tests.py | Letztes Feature der April-Roadmap. Schliesst die Roadmap ab. (A) Modul src/orchestrator.py: heuristisches should_orchestrate(msg) prueft 3 Kriterien (priorisiert): Bullet-Liste mit >=2 Bullets, >=2 distinkte Task-Kategorien, >=50 Woerter mit mind. 1 nicht-'other'-Kategorie. Force-Override moeglich. Liefert Diagnose-Dict mit reason, words, categories, has_bullets. (B) decompose(msg) — zwei-stufig: zuerst LLM (set_llm_invoker bekommt Haiku 4.5 als Decomposer mit deutschem System-Prompt + JSON-Output-Schema), Fallback heuristisch (Bullet-Punkte oder Saetze nach distinkter Kategorie). LLM-Invoker wird von web_server.py beim Import gesetzt — orchestrator.py kennt die ADAPTERS-Implementierung nicht direkt (saubere Schichten-Trennung). Decomposer-Modell ist konfigurierbare Konstante DECOMPOSITION_MODEL=('anthropic','claude-haiku-4-5-20251001') — bewusst guenstiges Modell, da nur Aufgabenzerlegung. (C) select_models(subtasks) augmentiert jeden Subtask via skill_database.get_best(skill_for_category, n=1) mit {model_ref, provider, model_id, display_name, rating, rationale, skill_used}. CATEGORY_TO_SKILL-Map identisch zur Pattern-Analyzer-Map (gleiche Bedeutung). (D) propose(msg, session_id, force) — Komplett-Pipeline: should_orchestrate -> decompose -> select_models -> Persistenz in <datalake>/patterns/orchestrator_proposals.json (atomar tmp+fsync+os.replace) -> heartbeat.enqueue_notification mit type='orchestration_proposal' und 3 Actions (yes/no/edit). (E) respond(proposal_id, 'yes'|'no'|'edit', edited_plan) setzt status accordingly: accepted/declined/edited. Bei 'edit' wird ein vom User bearbeiteter Plan akzeptiert (max 6 Steps, Validierung der Kategorien) und Modelle neu selektiert. (F) execute_step(proposal_id, step_index, prior_output) fuehrt einen Step ueber den LLM-Invoker aus. prior_output (von Step N-1) wird als Kontext im User-Prompt gepackt. Ergebnis wird im step_results-Array persistiert + via usage_logger.log_turn als category='orchestrated' geloggt (Pattern Analyzer kann daraus Patterns lernen) + extra-Feld {orchestration_proposal_id, step_index} fuer Traceability. Status transitioniert proposed -> running -> completed/error. (G) 6 neue Endpoints: POST /api/orchestrate/check (heuristische Pruefung ohne Decomposition, schnell), POST /api/orchestrate/propose (komplett-Plan + Heartbeat-Push), GET /api/orchestrate/proposals (Liste), GET /api/orchestrate/proposal/<id>, POST /api/orchestrate/respond (yes/no/edit, 400/404 Validation), POST /api/orchestrate/execute_step (sequentiell vom Frontend gesteuert). (H) Frontend-Pattern: Browser ruft /propose, zeigt Plan + 3 Buttons, bei 'yes' iteriert ueber subtasks und ruft /execute_step pro Step mit dem prior_output des vorigen Steps. Backend speichert Step-Ergebnisse, Frontend zeigt sie inkrementell. Live verifiziert: /check fuer einfache Anfrage 'Hi' -> orchestrate=False; fuer 'Recherchiere Stripe vs Adyen, schreib daraus einen Investment-Report und erzeuge eine Infografik' -> orchestrate=True mit 3 Categories. /propose mit derselben Anfrage liefert via Haiku-LLM-Decomposition 4 saubere Subtasks: Research(Opus-4-6/research_long) -> Analysis(Opus-4-6/reasoning) -> Writing(Opus-4-6/writing) -> Image(Imagen-4 oder gpt-image-1). Saubere Modell-Selektion pro Skill via skill_database. 75 neue Tests (Modul-Sanity + should_orchestrate-Cases inkl. force/empty/None + Heuristik-Decomposer + select_models + Heuristik-propose-Pipeline + respond-Lifecycle yes/no/edit + 6 Live-Endpoints + Wiring-Grep). Suite 1791/1793 (gleiche 2 pre-existing Pillow-Failures). ROADMAP-STATUS: alle 6 Features deployed (1+2: Lifecycle+Heartbeat, 3: Usage Logger, 4: Skill-Database+Benchmark-Fetcher, 5: Pattern Analyzer, 6: Orchestrator). System ist self-organizing: usage -> patterns -> automatisierte Vorschlaege; Orchestrator ergaenzt durch ad-hoc Multi-Skill-Routing.

[2026-04-27] FEATURE: Pattern Analyzer (Roadmap-Feature 5 April 2026) — wochentliche Mustererkennung in Usage-Logs mit User-Bestaetigung via Heartbeat-Push | src/pattern_analyzer.py (neu) + src/web_server.py + tests/run_tests.py | Erste Anwendung der Lifecycle+Heartbeat-Infrastruktur (Features 1+2). (A) Modul src/pattern_analyzer.py: liest letzte 2 Monate aus usage_logs/usage_YYYY-MM.jsonl, clustert Turns nach Tupel (agent, category, weekday, time_bucket). 5 time_buckets: morning (5-11), forenoon (11-13), afternoon (13-17), evening (17-22), night (22-5, wraps). Pattern-Schwelle: MIN_OCCURRENCES=3. (B) User-Message-Analyse: simpler Tokenizer + Stop-Word-Filter (~30 deutsche/englische Stop-Words), Top-5-Terme als signature; ohne Embedding-Library — robust und ohne externe Abhaengigkeit (search_engine-Embeddings haben aktuell HTTP-429 Circuit-Breaker, also Fallback ist eh noetig). (C) Pro qualifying-Cluster wird ein Pattern-Dict gebaut: id (pattern_<hex10>), description (deutsch, mit Themen), agent, category, weekday/weekday_label, time_bucket, occurrences, last_seen, suggested_skill (via CATEGORY_TO_SKILL-Map), suggested_model (via skill_database.get_best n=1), suggested_rationale, suggested_schedule {kind:weekly, weekday, hour, minute}, user_response (null|yes|no|later), later_threshold (10), occurrences_at_last_response, automation_active, cron_job_name, last_automation_run. (D) Persistenz in <datalake>/patterns/detected_patterns.json (atomar tmp+fsync+os.replace). Bei erneutem Lauf: bestehende Patterns werden ge-updatet (cluster_key-Match), User-Response + Threshold + automation_active bleiben erhalten. Resurface-Logik: Patterns mit user_response='later' werden wieder anbietbar wenn (occurrences_now - occurrences_at_last_response) >= later_threshold. (E) Notifications via heartbeat.enqueue_notification: type='pattern_suggestion' fuer neue/resurfaced Patterns mit description, target-Schedule, drei actions (yes/no/later) + komplettem Pattern-Dict im Body. type='pattern_automation_run' wenn ein aktivierter Pattern-Cron-Job triggert. (F) respond(pattern_id, 'yes'|'no'|'later'): bei 'yes' -> activate_pattern_locked() registriert das Pattern als heartbeat.schedule_cron mit weekly_at(weekday, hour, 0)-Predicate; bei 'no' -> Deaktivierung; bei 'later' -> threshold-Counter zuruecksetzen. (G) Heartbeat-Integration: register_with_heartbeat() registriert sich beim Server-Start als Cron-Job 'pattern_analyzer_weekly' mit weekly_at(6, 22, 0) — Sonntag 22:00 Lokalzeit Sao Paulo (Python-weekday 6 = Sonntag). (H) 3 neue Endpoints: GET /api/patterns (alle erkannten Patterns + count), POST /api/patterns/respond {id, response} (yes/no/later, 400 bei invalidem Wert, 404 bei unbekannter ID), POST /api/patterns/analyze {push} (manueller Lauf mit optionalem Push-Suppress fuer Tests). Live verifiziert: erster Lauf auf den 68 echten Usage-Log-Eintraegen liefert 5 qualifying Patterns (z.B. "11x search-Anfrage an Agent 'signicat' jeden Montag nachmittags — Themen: find, test", "29x other-Anfrage an Agent 'privat' jeden Montag nachmittags"). respond('later'/'no'/'yes')-Pipeline live durchgespielt. 30 neue Tests (Modul-Sanity + time_bucket-Klassifikation + key_terms-Tokenizer + analyze_logs-Schema + Pattern-Schema + respond-Lifecycle + 3 Live-Endpoints + Wiring-Grep). Suite 1716/1719 (gleiche 2 pre-existing Pillow + 1 pre-existing API-Gaps-flaky). Naechster Schritt: Feature 6 (Orchestrator).

[2026-04-27] FEATURE: Context Lifecycle Engine + Heartbeat-Daemon (Roadmap-Features 1+2 April 2026) — Event-Bus fuer Turn-Hooks + Push-Pump fuer Browser-Notifications + Cron-Registry | src/lifecycle.py (neu) + src/heartbeat.py (neu) + src/web_server.py (refactored) + tests/run_tests.py | Externe Spec-Datei (assistantdev_lifecycle_heartbeat_task.md) war nicht auffindbar — Spec aus dem Roadmap-Header und den schon existierenden Anker-Punkten abgeleitet (Feature 3 erwaehnt AFTER_TURN namentlich, Feature 4 schreibt in pending_notifications.jsonl und braucht den Konsumenten). (A) Neues Modul src/lifecycle.py: schlanker In-Process Event-Bus mit 7 benannten Lifecycle-Punkten als Konstanten (BEFORE_TURN, AFTER_TURN, ON_AGENT_SWITCH, ON_SESSION_START, ON_SESSION_END, ON_SUBAGENT_DELEGATION, ON_TURN_ERROR). API: on(event, handler, label) -> sub_id, off(event, sub_id), emit(event, ctx) -> stats-dict, list_subscribers(), reset(). Bewusst nicht-async (kein asyncio, kein Event-Loop) — passt zu Flasks Thread-pro-Request-Modell. emit() ruft alle Handler synchron-aber-isoliert auf (try/except pro Handler), liefert Stats {handlers_total, handlers_ok, handlers_errored, errors[]}. Failing Handler propagiert NIE — andere Subscriber laufen weiter, /chat-Request ist niemals betroffen. ctx kann non-dict sein (wird automatisch in {value: ...} gewrapped). (B) Neues Modul src/heartbeat.py: Daemon-Thread mit zwei Aufgaben: (1) Notification-Pump — drained periodisch <datalake>/config/pending_notifications.jsonl in eine In-Memory-Queue, archiviert konsumierte Eintraege in pending_notifications.archive.jsonl (truncate-after-archive verhindert Duplikate). (2) Cron-Registry — schedule_cron(name, schedule_fn, job_fn, run_immediately) registriert wiederkehrende Jobs. last_run-Persistenz in heartbeat_cron_state.json fuer Server-Restart-Resilienz. Predicates: every_n_seconds(n), daily_at(hour, minute), weekly_at(weekday, hour, minute). Daemon-Tick: 60s. Zusaetzlich direkter enqueue_notification(payload) fuer Subsysteme die nicht ueber Datei gehen wollen. (C) Refactor Feature 3: Der bisherige _log_chat_turn-Wrapper ist jetzt zwei-stufig — _usage_logger_after_turn(ctx) ist der Lifecycle-Subscriber, _emit_after_turn(...) ist der einheitliche Aufruf an allen Call-Sites. Beim Modul-Import wird der Subscriber an AFTER_TURN registriert. Wenn lifecycle.py nicht verfuegbar ist -> Fallback auf direkten Logger-Aufruf (Backward-Kompatibilitaet zu Feature 3). Existierende Call-Sites bleiben unveraendert (Legacy-Alias _log_chat_turn delegiert auf _emit_after_turn) — kein Risiko fuer chat()/process_queue_worker/subagent_confirm. (D) Endpoints: GET /api/heartbeat/status (Diagnose: queue_size, cron_jobs, lifecycle-events, alle Subscriber pro Event), GET /api/heartbeat/notifications?since_id=&limit= (Browser-Polling, markiert Items als 'delivered'), POST /api/heartbeat/ack {ids:[]} (entfernt aus Pending-Queue, 400 bei nicht-Liste). (E) Heartbeat-Daemon wird beim Server-Start automatisch gestartet — der existierende benchmark_fetcher-Scheduler (Feature 4) bleibt unangetastet (laeuft mit eigenem Thread). Spaetere Features (Pattern Analyzer, Orchestrator) sollen Cron-Jobs ueber heartbeat.schedule_cron(...) registrieren statt eigene Threads zu starten — Konvention dokumentiert. Live verifiziert: Server-Startup-Logs zeigen "[heartbeat] Daemon gestartet" + "[lifecycle] usage_logger als AFTER_TURN-Handler registriert". /api/heartbeat/status liefert AFTER_TURN-Subscriber {id:1, label:'usage_logger.log_turn'}. End-to-End: /chat-Turn -> lifecycle.emit('AFTER_TURN', ctx) -> usage_logger schreibt JSONL-Eintrag -> turns_today inkrementiert. Failing-Handler-Test: emit() mit ZeroDivision-Subscriber liefert handlers_errored=1 + traceback-snippet, andere handler laufen weiter. 53 neue Tests (Modul-Sanity + on/emit/off + Failing-Handler-Isolation + Notification-enqueue/fetch/ack + Duplicate-ID-Handling + Cron-Predicates + schedule_cron+run_immediately + 3 Live-Endpoints + Wiring-Grep + E2E /chat). Suite 1657/1659 (gleiche 2 pre-existing Pillow-Failures). Naechste Schritte laut Roadmap: Feature 5 (Pattern Analyzer) baut auf Lifecycle (AFTER_TURN-Subscriber sammelt Patterns) + Heartbeat (Cron Sonntag 22:00, Push-Vorschlaege via enqueue_notification). Feature 6 (Orchestrator) nutzt Lifecycle (BEFORE_TURN-Subscriber inspect Task-Komplexitaet) + Skill-Database (best-of pro Teilaufgabe).

[2026-04-27] FIX: POST /api/messages/mark-read idempotent fuer conversation_id (Frontend-QA-Probe gruen) | src/web_server.py + tests/run_tests.py | User-Beobachtung via AdminQAChecklist.tsx-Probe: 'mark-read akzeptiert conversation_id' schlug fehl. Backend akzeptierte conversation_id technisch, gab aber bei Miss 404 mit Body 'no messages matched' zurueck — die Probe pattern-matchte nur 'conversation' im error-Body und wertete 404 als 'Endpoint noch nicht implementiert'. Fix: Wenn conversation_id explizit (oder mode=='conversation') geliefert wird und keine Messages matchen, liefert der Endpoint jetzt idempotent 200 mit count=0 + note 'conversation_id accepted but no messages matched (idempotent)'. Strict-404-Pfad bleibt nur fuer ungueltige message_id (mit verbessertem Hint 'try conversation_id for chat sources'). Verhalten matcht Frontend-Erwartung: conversation als gelesen markieren ist immer ok, auch wenn die Conv leer/verschwunden ist. Live verifiziert: __qa_probe__ -> 200 count=0; echte WhatsApp-cid -> 200 count=50; ungueltige message_id -> 404. 7 neue Tests (Source-Greps + alle 3 Live-Pfade), bestehender Test (api-gaps live) angepasst auf neues Verhalten. Suite 1575/1577 (gleiche 2 pre-existing Pillow).

[2026-04-27] FIX: Permissions-Detection + Matrix (BACKEND_TODOs PERMISSIONS_DETECTION + PERMISSIONS_MATRIX_EMPTY) | src/web_server.py + tests/run_tests.py + frontend/src/components/FIXED_PERMISSIONS_*.md | (A) /api/permissions vorher: Canva/Google/macOS-Calendar/Contacts/Mail standen als 'missing' bzw. 'unknown' obwohl tatsaechlich verbunden — Detection prufte nur OAuth-File-Existenz oder gar nichts. Fix: _canva_connected liest models.json -> canva.access_token + refresh_token, _google_calendar_connected unterstutzt mehrere Token-Pfade plus models.json -> google_calendar, _macos_mail_connected nutzt _apple_mail_envelope_probe() fur echte SQLite-Probe, _macos_contacts_connected prueft AddressBook-Verzeichnis lesbar, _macos_calendar_connected prueft ~/Library/Calendars. Neue instructions_markdown pro Provider mit Probe-Beschreibung + FDA-Anleitung. Slack-Eintrag entfernt aus /api/permissions UND /api/oauth-status (User-Entscheidung: kein Slack-OAuth mehr). (B) /api/permissions_matrix lieferte zwar agents+cells aber Frontend erwartete 'resources'-Field — daher Empty-State im UI. Fix: Response um 'resources' erweitert (kanonisch), 'scopes' bleibt als Alias. Default-Resources [mail, whatsapp, imessage, kchat, calendar, contacts, webclips, memory, memory_shared] werden auch ohne ACL-Eintrag materialisiert. Agents Union aus access_control.json + Disk (AGENTS_DIR), damit neu angelegte Agents sofort in der Matrix stehen. Cells haben crossRead-Feld. Live: 11 agents x 9 resources = 99 cells (vorher 11x4=44 nur). 21 neue Tests, Suite 1568/1570 (gleiche 2 pre-existing Pillow-Failures). Frontend-Repo: BACKEND_TODO_PERMISSIONS_DETECTION_2026-04-27 + BACKEND_TODO_PERMISSIONS_MATRIX_EMPTY_2026-04-27 -> FIXED, gepushed. Plus User hat zwischenzeitlich Full Disk Access fur den Server-Process gegeben — Apple Mail Envelope-Index jetzt zugaenglich (116200 messages, 4305 unread), iMessage chat.db ok=true. Damit auch Read-State-Sync gefixt: vorher 207/241 (86%) Privat-Mails als ungelesen markiert, jetzt 3/192 (2%); Signicat 125/383 (33%) -> 3/382 (1%).

[2026-04-27] FIX: Junk-Filter erweitert um numerierte Marketing-Subdomains | src/web_server.py + tests/run_tests.py | User-Beobachtung: Tonnen ungelesener Newsletter im Privat- und Signicat-Posteingang trotz Junk-Filter. Diagnose: 0 von 804 Mails als is_junk markiert, weil das substring-basierte _MSG_JUNK_SENDER_PATTERNS-Set Marketing-Subdomains mit Zahl-Suffix nicht abdeckte (em1.cloudflare.com, ses2.unternehmenswelt.de, em0.shopify.com etc.). Fix: (A) Pattern-Liste erweitert um @mkt., @news., @nl., @info., @updates., @ses, @em-marketing usw. plus internationale Body-Signale fuer "View in browser" (portugiesisch "ver no navegador", spanisch "ver en el navegador", italienisch, franzoesisch, deutsch). (B) Neue Regex _MSG_JUNK_SENDER_REGEX als Fallback fuer numerierte Subdomains: @(em|news|nl|mkt|...)[0-9]*\\. — matcht em1, ses2, news0 etc. Live-Verifikation: vorher 0/804 junk, nach Fix 579/1327 junk (44%) — em@em1.cloudflare.com jetzt korrekt is_junk=True. Default GET /api/messages?junk=hide entfernt diese aus dem Listing. 13 neue Tests (Pattern-grep + Regex-Sanity-Cases positiv/negativ + Live-Treffer-Quote). Suite 1549/1551 (gleiche 2 pre-existing Pillow). UNRELATED Folge-Beobachtung: User klagt auch ueber zu viele "ungelesen"-Markierungen — das ist ein separates macOS-FDA-Permission-Problem (Server-Process /Applications/Xcode.app/Contents/Developer/usr/bin/python3 hat keinen Full-Disk-Access fuer ~/Library/Mail/V*/MailData/Envelope Index). Probe-Endpoint /api/probe-fda zeigt den exakten Pfad fuer Systemeinstellungen → Datenschutz & Sicherheit → Festplattenvollzugriff.

[2026-04-27] FEATURE: Skill-Database + Benchmark-Fetcher (Roadmap-Feature 4 April 2026) — zentrale Faehigkeits-DB mit Tier-basierten Defaults und wochentlichem Update-Schedule | src/skill_database.py (neu) + src/benchmark_fetcher.py (neu) + src/web_server.py + tests/run_tests.py | Architektur: (A) Neues Modul src/skill_database.py mit 13 Skills (research_long, writing, coding, reasoning, instruction_following, image_understanding, image_generation, video_generation, translation, document_analysis, web_search, speed, cost_efficiency) auf 1-5-Skala, 0 = nicht unterstuetzt. Storage als JSON in <datalake>/config/skill_database.json mit atomarem Write (tmp + fsync + os.replace). (B) classify_tier ordnet Modelle 9 Tier-Buckets zu (flagship_reasoning, flagship, mid, small_fast, research_specialist, code_specialist, local, image_gen, video_gen) basierend auf Provider/Model-ID/Name-Heuristiken — jeder Tier hat ein Default-Profil das beim Bootstrap angewendet wird. Beispiele: claude-opus-4-7 -> flagship_reasoning (5/5/5/5/5), claude-haiku-4-5 -> small_fast (3/3/3/3/4 + speed:5 + cost_efficiency:5), codestral-latest -> code_specialist (coding:5, research_long:2), perplexity sonar-deep-research -> research_specialist (research_long:5, web_search:5), Veo 3.1 -> video_gen (video_generation:5, alles andere 0). (C) bootstrap_from_models() initialisiert die DB aus models.json — alle 40 Chat-Modelle + Image-Submodelle (gpt-image-1, imagen-4.0) + Video-Submodelle (veo-3.1) werden gemappt, insgesamt 43 Eintraege. Idempotent: bei erneutem Lauf bleiben User-Edits + bereits gefetchte Werte erhalten, nur fehlende Skills/Modelle werden ergaenzt. (D) get_best(task, n=3) sortiert nach (rating, speed, cost_efficiency) absteigend — Codestral + GPT-5.3 Codex + Opus 4.6 fuer 'coding'; Veo 3.1 fuer 'video_generation'; Imagen-4 + GPT-Image-1 fuer 'image_generation'; Sonar Deep Research fuer 'web_search'. (E) apply_updates() persistiert Diffs in <datalake>/config/skill_database_changes.jsonl — pro Update: timestamp, model_ref, skill, old, new, source. Idempotent (gleicher Wert -> 0 Aenderungen), validiert Rating-Range 0-5, kennt nur whitelisted SKILLS, ignoriert unbekannte model_refs. (F) Neues Modul src/benchmark_fetcher.py mit Pluggable-Source-Architektur: register_source(name, fn) + fetch_from_artificialanalysis() (best-effort __NEXT_DATA__-Scraping mit 32 Regex-Patterns fuer Modell-Namen-Resolution). Bei Schema-Aenderung der Quelle -> graceful empty list, kein Crash. Score-Mapping aa->intern: quality_index/intelligence (0-100) -> reasoning, coding -> coding, tokens_per_second -> speed (>=150 tok/s -> 5), price_per_million -> cost_efficiency (<=0.5 USD -> 5). (G) Scheduler-Thread im Daemon-Mode prueft stuendlich ob Wochen-Slot faellig (Montag, ab 06:00 Lokalzeit America/Sao_Paulo, mindestens 6 Tage seit letztem Lauf). State in config/benchmark_fetcher_state.json (last_run, last_status, history-Ring der letzten 20 Laufe). (H) Bei signifikanten Aenderungen (>=1 Punkt Diff) wird ein Heartbeat-Notification-Kandidat in config/pending_notifications.jsonl geschrieben — Heartbeat-System (Roadmap-Feature 1+2) wird das spaeter konsumieren. (I) Wiring in web_server.py: Bootstrap on Startup + Scheduler-Start in den Import-Block, beide isoliert mit try/except (Import-Fehler -> No-Op-Stub, Server startet trotzdem). 4 neue Endpoints: GET /api/skills/database (komplette DB), GET /api/skills/best?task=X&n=3 (Top-N + Rationale), GET /api/skills/model?id=provider/model_id (Einzel-Modell, 404 bei missing, 400 bei ungueltigem Format), POST /api/skills/refresh (manueller Fetcher-Trigger, Default force=true). Live verifiziert: GET /api/skills/best?task=coding liefert Codestral + gpt-5.3-codex + Opus-4-6 als Top-3 mit Rationale-Strings. POST /api/skills/refresh liefert sauberes Summary-Dict mit sources-Status. artificialanalysis.ai liefert aktuell kein __NEXT_DATA__-JSON mehr (Schema-Aenderung) -> source_ok:false reason:"no __NEXT_DATA__", aber die Pipeline ist robust. 91 neue Tests (Modul-Sanity + Tier-Klassifikation fuer 13 Modelle + Bootstrap-Coverage + get_best-Sortierung + apply_updates-Diff/Rollback/Validation + Endpoint-Smoketest + Wiring-Grep). Suite 1534/1536 (gleiche 2 pre-existing Pillow-Failures). Naechste Schritte laut Roadmap-Sequenz: Feature 1+2 (Lifecycle + Heartbeat) damit pending_notifications.jsonl verarbeitet werden kann, dann Feature 5 (Pattern Analyzer).

[2026-04-27] FIX: Email-Detail Slash-in-Subject + Read-Status-Latenz | src/web_server.py + tests/run_tests.py | (A) Slash-Bug: /api/messages/conversation/<conv_id> hat fuer Mails mit "/" im Subject (z.B. "Fall Winter 26/27") HTTP 404 zurueckgegeben, weil Flasks Default-Konverter <conv_id> Slashes verschluckt — Frontend hatte dann keine Thread-Liste, kein Detail-Fetch, kein Body. Fix: Route-Konverter auf <path:conv_id> umgestellt. Verifiziert live mit Mail "T-Shirts — Fall Winter 26/27" (em:contato@mkt.osklen.com|...26/27): vorher 404, jetzt 200 mit 2 Messages, body_text 1930 chars im /api/messages/<id>. (B) Read-Status-Latenz: _APPLE_MAIL_READ_TTL von 90s auf 20s gesenkt. Apple-Mail-Envelope-Index-Reads sind ~ms-billig (read-only SQLite), aggressiveres Refresh kostet kaum was. Worst-case Sync-Latenz beim Markieren-als-gelesen in Apple Mail damit von 90s auf 20s. _MSG_CACHE_TTL_SECONDS bleibt auf 60s (das ist nur file-listing, read-state wird live pro Request appliziert). 6 neue Tests (Slash-Konverter + TTL-grep + Live: slashy cid -> 200 + body_text >= preview). Suite 1421/1423 gruen (gleiche 2 pre-existing Pillow-Failures).

[2026-04-27] FEATURE: Usage Logger (Roadmap-Feature 3 April 2026) — passives Turn-Logging als Datenbasis fuer Pattern Analyzer + Orchestrator | src/usage_logger.py (neu) + src/web_server.py + tests/run_tests.py | Erstes Feature aus der April-Roadmap. Strategischer Grund: Pattern Analyzer (Feature 5) braucht laut Spec mindestens 2 Wochen Logs, bevor er sinnvoll Muster erkennen kann — also so frueh wie moeglich anfangen zu sammeln, parallel zu den anderen Features. Architektur: (A) Neues Modul src/usage_logger.py mit threadsicherer queue.Queue + Daemon-Writer-Thread — log_turn() ist fire-and-forget, blockiert /chat niemals (Queue-Full -> drop, kein Wait). Bei voller Queue (10000) wird verworfen statt zu warten. (B) Storage als JSONL, monatlich rotiert: <datalake>/usage_logs/usage_YYYY-MM.jsonl. Felder pro Eintrag: timestamp (ISO 8601, Timezone America/Sao_Paulo via zoneinfo, Fallback Systemzeit), session_id, agent, sub_agent, provider, model, user_message, assistant_response, duration_seconds, tokens_estimated.{input,output} (Heuristik len/4), category. (C) classify_task: Keyword-Klassifikator mit 12 Kategorien (research, coding, writing, email, document, calendar, image, video, translation, analysis, search, other), Reihenfolge ist Prioritaet bei Mehrfach-Treffern (image vor research weil "Bild generier" oft mit "fuer Marketing-Material" kombiniert wird). (D) Wiring in web_server.py via _log_chat_turn-Wrapper an 4 Call-Sites: /chat-Handler (Happy-Path), process_queue_worker (queued Turns), /api/subagent_confirm (decline-Pfad), /api/subagent_confirm (confirmed-Delegation, mit sub_agent-Feld). Wrapper ist try/except-isoliert — niemals exception-propagating. Sub-Agent-Confirmation-Requests + Error-Responses werden bewusst NICHT geloggt (kein vollstaendiger Turn). (E) Neuer Endpoint GET /api/usage/summary liefert: turns_today, turns_this_week (ab Montag), turns_this_month, top_categories (Top 5), top_model_per_agent, avg_turn_duration_seconds, logfile-Pfad. Bei Modul-Importfehler -> graceful Stub mit available:false statt 500. Live verifiziert: nach echtem /chat-Turn (Haiku) inkrementiert turns_today, JSONL-Datei enthaelt Eintrag mit korrektem provider/model/category, Summary-Endpoint liefert konsistente Aggregate. 53 neue Tests (Klassifikator-Sanity + Schreibpipeline + Summary-Shape + Endpoint-Smoketest + Wiring-Grep + Live-Increment). Suite 1417/1419 (gleiche 2 pre-existing Pillow-Failures). Naechster Schritt: Feature 4 (Skill-Database).

[2026-04-27] FIX: DELETE /agents nimmt reason aus Body/Header/Query, URL-decoded | src/web_server.py + tests/run_tests.py | Frontend-DELETE-Wrapper schickt bei manchen Proxies keinen JSON-Body — der Reason fuer den Confirm-Dialog kam deshalb nicht ins _meta.json. Fix: drei Kanaele unterstuetzt mit Prioritaet Body > X-Archive-Reason Header > ?reason= Query. Header wird URL-decoded (HTTP-Konvention fuer Non-ASCII). Wenn keine Quelle gesetzt ist, bleibt reason: None statt vorheriger Default 'user_delete'. _archive_agent-Signatur entsprechend angepasst (reason=None). Live verifiziert: alle 4 Akzeptanzkriterien (kein-reason, Header decoded, Query, Body-priority). 8 neue Tests, Suite 1364/1366 (gleiche 2 pre-existing Pillow-Failures). Frontend-Repo: BACKEND_TODO_DELETE_AGENT_REASON -> FIXED.

[2026-04-27] FEATURE: Agent-Lifecycle robust — CREATE+RENAME+DELETE+RESTORE mit komplettem Storage-Handling | src/web_server.py + tests/run_tests.py + frontend(separat) | User-Wunsch: "Wenn ich Agents umbenne, neue hinzufuege oder loesche soll das in ALLEN Orten sauber abgefahren werden — Dateinamen, Storage, alles. Insbesondere fuer Loeschen brauchen wir einen sauberen Archive-Prozess." Vorher war das fragil: CREATE schrieb nur die .txt (memory blieb leer), RENAME aenderte nur die .txt (Storage-Dir blieb unter altem Namen, Konversationen verwaist), DELETE benannte nur die .txt um. Jetzt: (A) Helper _default_agent_system_prompt liefert sauberen Default-Prompt mit OUTPUT-Block-Konvention. (B) _create_agent_storage_skeleton legt memory/ + working_memory/_manifest.json + memory/contacts.json (mit Schema) atomar an, alles mit fsync(). (C) _move_agent_storage benennt Storage-Root um — atomar mit Rollback bei Fehler. (D) _archive_agent verschiebt Agent-Prompt + Storage komplett nach <base>/.deleted_agents/<slug>_<ts>/ mit _meta.json (Stats: emails, conversations, archived_at, reason). Soft-Delete-Konvention (CLAUDE.md): nichts wird unlinked. (E) _restore_agent — inverse Operation. POST /agents/restore/<archive_id> + GET /agents/archive Endpoints. (F) Subagent-CRUD analog konsistent gemacht (Subagents teilen Parent's storage, also kein Storage-Move, aber sauberes File-Handling + fsync + Archivierung). End-to-end live verifiziert: CREATE "lifecycle_test" -> alle 4 Files angelegt; RENAME -> Storage-Dir migriert; DELETE -> Archive mit storage_stats:{emails:0, conversations:1}; RESTORE -> alles zurueck inkl. Konversation; GET /agents/archive listet Archive. 26 neue Tests inkl. Live-Smoketest, Suite 1358/1360. Frontend-TODO im separaten Repo fuer Confirm-Dialog + Papierkorb-View.

[2026-04-27] FIX: 3 Backend-TODOs erledigt — Agent-Persistenz, Cleanup global, iMessage-FDA-Diagnose | src/web_server.py + tests/run_tests.py | Frontend hatte 3 BACKEND_TODOs neu geoeffnet. (1) AGENT_CREATE_NOT_PERSISTED: User legt "Nayara" an, weg nach Reload. Root-Cause: `_`-Heuristik in get_parent_agent gruppierte jeden Agent mit Underscore (z.B. "nayara_v2") als Subagent — wenn das vermutete Parent-File ("nayara.txt") nicht existierte, kaperte ein synthetic-parent-Eintrag in get_agents() den Top-Level-Slot. Fix: Orphan-Children werden jetzt direkt als Top-Level-Eintraege gehoben statt unter synthetic-parent versteckt. Plus POST /agents schreibt mit fsync() + Info-Log "[AGENT-CREATE] agent_created name=...". 409-Logik fuer Duplikat-Slugs war schon korrekt. (2) CONVERSATION_CLEANUP_GLOBAL: Endpoint POST /api/conversations/cleanup lief schon global, aber Response-Shape fehlte Per-Agent-Breakdown. Hinzu: agents:[{agent, would_delete, ids}] sortiert nach agent-name + total-Alias. Frontend kann jetzt im Confirm-Dialog "X in privat, Y in signicat" zeigen. (3) IMESSAGE_INBOX_EMPTY: nicht ein Code-Bug — FDA-Permission fehlt fuer den Server-Prozess (/usr/bin/python3 in /Applications/Xcode.app/.../python3). _imsg_probe_access liefert jetzt zusaetzlich `fix`-Feld mit konkretem User-Action-Text + Pfad zum Binary; /api/messages/sources expose access_fix + access_binary. Live verifiziert: "Nayara V2" -> top-level erscheint. Cleanup-Response total=115 in 5 Agents. iMessage-Probe gibt: "Full-Disk-Access fuer den Server-Prozess fehlt. Oeffne System-Einstellungen -> Datenschutz & Sicherheit ...". 14 neue Tests, Suite 1327/1329 (gleiche 2 pre-existing Pillow-Failures). Frontend-Repo: 3 BACKEND_TODOs -> FIXED_*_2026-04-27.md.

[2026-04-27] FEATURE: 2 Backend-TODOs erledigt — body_html + voll-strukturierte E-Mail-Attachments | src/web_server.py + src/email_watcher.py + scripts/backfill_email_html.py + tests/run_tests.py + EmailWatcher.app rebuilt | Frontend hatte BACKEND_TODO_EMAIL_HTML_BODY.md + BACKEND_TODO_EMAIL_ATTACHMENTS.md geschrieben — beide adressiert. (A) HTML-Body: Email-Watcher extrahiert text/html-Part jetzt SEPARAT (statt zu text-strippen) und schreibt eine .html-Companion-Datei neben jede .txt; plus EML-Source-Header zeigt auf das Original-.eml in email_inbox/processed/ fuer Reparse-Fallback. _msg_normalize_email_content liest body_html aus Companion oder reparst .eml. /api/messages/<id> reparst jetzt mit only_header=False um body_text + body_html mitzuliefern. (B) Attachments: _MIME_BY_EXT-Map fuer 25 Endungen, Anhaenge bekommen voll-strukturierte Objects mit id (URL-safe Hash), name, mime, size (stat), url (/api/messages/<id>/attachments/<att_id>), inline=False. Backwards-Compat: attachment_filenames bleibt als String-Liste. (C) Download-Endpoint /api/messages/<msg_id>/attachments/<att_id> streamt File via send_from_directory mit korrektem Content-Type, as_attachment=True, Pfad-Traversal-Schutz via os.path.realpath-Checks. (D) Watcher-Binary rebuilt mit PyInstaller (build_emailwatcher/EmailWatcher.spec), nach ~/Applications/EmailWatcher.app/ deployed, com.moritz.emailwatcher LaunchAgent neu geladen. Live verifiziert: synthetische Mail durch den Watcher -> .html-Companion (120 bytes) + .txt (613 bytes) + Anhang in attachments/ -> /api/messages/<id> liefert body_html komplett mit Tags + attachments[] mit url -> Download-Endpoint streamt PNG mit Content-Type=image/png. (E) Backfill-Skript scripts/backfill_email_html.py fuer optionale Backfill der ~22k existierenden .txt-Mails (Heuristik via Filename-Match — primaer fuer kuenftige Mails wichtig). 19 neue Tests, Suite 1313/1315 (gleiche 2 pre-existing Pillow-Failures). Frontend-Repo: 2 BACKEND_TODOs -> FIXED_*_2026-04-27.md gepusht.

[2026-04-27] FIX: Message-Dashboard Cleanup — kChat raus, "Sonstige"-Label, Phantom-Mail-Filter, Junk-Toggle | src/web_server.py + tests/run_tests.py + frontend(separat) | User-Feedback (Screenshots): (A) Doppelte Kategorien — "Privat" + "Sonstige" waren redundant. Fix: email_standard heisst jetzt "Sonstige" statt "Standard / uebrige E-Mails". (B) kChat raus aus _MSG_SOURCES — User nutzt es nicht aktiv, Watcher hatte Token-Probleme, war leerer Noise im Dashboard. Watcher-LaunchAgent bleibt, nur die Dashboard-Spalte ist weg. (C) Sortierung: E-Mail-Quellen oben, Messaging darunter (war schon implizit, jetzt durch Reihenfolge in _MSG_SOURCES garantiert). (D) Phantom-Thread-Bug "(unbekannt) / kein Betreff" mit 23 ungelesenen — Root-Cause: 24 Mails vom Email-Watcher mit komplett leeren Headers (Von:/An:/Betreff: alle leer) kollabierten zu EINEM conversation_id "em:|(kein betreff)" — Klick + Mark-as-Read gingen ins Leere weil AppleScript-Match-Key auch leer war. Fix in _msg_normalize_email_content: keine sender_email + keine raw_subject -> return None (raus aus dem Inbox-Stream). Plus: conversation_id ist jetzt per-File-Hash wenn sender_email leer, damit auch die anderen 63 Header-armen Mails nicht mehr kollabieren. (E) Junk-Mail-Toggle: /api/messages?junk=hide|show|only und /api/messages/sources?junk=... — Default hide, sources liefert jetzt zusaetzlich junk_count + junk_unread + total_including_junk pro Quelle, damit Frontend einen "X Newsletter"-Badge zeigen kann. Live verifiziert: Sources gehen von 7 auf 6 (kchat raus), email_privat geht von 499 auf 240 visible (212 junk versteckt), keine Phantom-Mails mehr. 14 neue Tests, Suite 1296/1298 (gleiche 2 pre-existing Pillow-Failures). Frontend-TODO im separaten Repo fuer Junk-Toggle-UI committet.

[2026-04-27] FIX: Perplexity max_tokens per-Modell statt pauschal 8000 — Sonar Deep Research nicht mehr truncated | src/web_server.py + tests/run_tests.py | User-Feedback (zu Recht): "Bei Perplexity Deep Research und Sonar Reasoning Pro kann ich mir nicht vorstellen, dass die einen Token von 8000 haben — das sind mehrseitige Research-Reports". Recherche bestaetigt: pauschal 8000 war fuer Sonar Pro zufaellig richtig (echtes Hard-Cap), aber fuer Sonar Deep Research und Sonar Reasoning Pro massiv zu wenig. Realitaet (verifiziert via docs.perplexity.ai + Community-Truncation-Reports): sonar=4000 (lite, kurze Antworten), sonar-pro=8000 (Hard-Cap), sonar-reasoning-pro=16000 (Reasoning-Tokens zaehlen mit), sonar-deep-research=32000 (mehrseitige Papers, sonst Truncation). _provider_max_tokens differenziert jetzt per-Modell. Live verifiziert: sonar-pro (max=8000) und sonar-reasoning-pro (max=16000) liefern korrekt. 4 neue Tests, Suite 1283/1285.

[2026-04-27] FIX: max_tokens auf realistische Werte pro Provider — 20-Seiten-Dokumente moeglich | src/web_server.py + tests/run_tests.py | User-Frage: "Reichen 8192 Tokens fuer 20-seitige Dokumente?" Ehrliche Antwort: nein. Berechnung: 20 Seiten × 300 Wörter ≈ 7.800 Tokens reiner Text + ~2.500 Tokens JSON-Overhead = ~10k Tokens minimum, eher 12-15k bei detaillierten Specs. 8192 ist genau auf der Kante. Fix: zentrale Helper-Funktion `_provider_max_tokens(provider, model_id)` mit Provider-spezifischen offiziellen Output-Limits — Anthropic Claude 4 Opus/Sonnet 32000 (Haiku 4.5 Fallback 8192), OpenAI GPT-5.x 16384, DeepSeek V4 16000 (Reasoning + Content), Mistral 16384, Perplexity 8000, Ollama 16384. Bei Anthropic max_tokens > 16000 erzwingt das SDK sonst Streaming wegen 10-min-Timeout-Heuristik — Fix via per-Request `timeout=1800.0`. Bei API-Ablehnung (z.B. neue Modell-Variante mit kleinerem Limit) faellt call_anthropic auf 8192 zurueck. Stress-Test: Opus 4.7 mit max_tokens=32000, Spec mit 114 Inhaltsbloecken (22.359 Zeichen JSON), 43 KB DOCX in 128s erzeugt — frueher waere das bei ~80 Bloecken truncated. 19 neue Tests, Suite 1279/1281 (gleiche 2 pre-existing Pillow-Failures).

[2026-04-27] FIX: CREATE_FILE schlug bei laengeren Dokumenten silent fehl — max_tokens-Bump + Truncation-Detection | src/web_server.py + tests/run_tests.py | User-Bug: Architektur-Doc-Export gab Fehler "Datei-Erstellung fehlgeschlagen: JSON-Format ungueltig". Root-Cause: max_tokens=4096 in call_anthropic/call_openai/call_ollama war fuer DOCX-Specs mit Inhaltslisten zu klein — der LLM truncated mid-JSON, der Bracket-Depth-Parser fand kein passendes `}]` und droppte den Block silent. User sah `[CREATE_FILE:...` plus Bruchstuecke der content-Liste im Chat, ohne klare Erklaerung. Fix: (A) max_tokens=4096 -> 8192 fuer Anthropic/OpenAI/Ollama/Mistral in beiden duplizierten Adapter-Bloecken (713/2353, 720/2360, 736/2376, 837/2477). DeepSeek/Perplexity bleiben bei 8000 (war ohnehin schon ok). (B) Truncation-Detection: nach extract_blocks scannen wir uebriggebliebene `[CREATE_FILE:`-Reste und ersetzen sie durch eine klare Fehlermeldung "CREATE_FILE-Block wurde mid-JSON abgeschnitten (max_tokens-Limit erreicht)". (C) Generische "JSON-Format ungueltig"-Maskierung entfernt — User sieht jetzt den echten Parser-Fehler mit Position. (D) Fehlgeschlagene JSON-Specs werden in `~/AssistantDev/logs/create_file_failures.log` persistiert fuer Post-Mortem. Live verifiziert: alle 4 File-Typen (docx/xlsx/pdf/pptx) erstellen Files in `claude_outputs/` (37 KB DOCX mit Tabelle+Bullets durchgegangen). 10 neue Tests, Suite 1267/1269 (gleiche 2 pre-existing Pillow-Failures).

[2026-04-26] FEATURE: Ollama 14B-Modelle — Qwen 3 14B + DeepSeek R1 14B lokal pulled + im Dropdown | config/models.json + src/web_server.py + tests/run_tests.py | Beide Modelle (~9 GB pro Stueck) via `ollama pull qwen3:14b` und `ollama pull deepseek-r1:14b` lokal gezogen (Mac M4, 16GB RAM, Q4_K_M). Pulls liefen erst per Bash background, einer ist netzwerkbedingt abgebrochen — neugestartet mit `nohup` (resilient gegen Shell-Teardown), Ollama setzte mit den vorhandenen Partial-Blobs auf. Nach den Pulls live verifiziert via call_ollama: `qwen3:14b` -> "Bestätigt", `deepseek-r1:14b` -> "Ja" (OpenAI-kompatible API auf 127.0.0.1:11434/v1). models.json: ollama-Provider um qwen3:14b + deepseek-r1:14b ergaenzt (8B bleibt fuer Geschwindigkeit). MODEL_DISPLAY um qwen3:14b erweitert (R1:14B war bereits vom April-Catalog-Refresh drin). 4 neue Tests: 2 grep auf MODEL_DISPLAY/models.json + 2 live gegen Ollama Daemon /api/tags. Suite 1257/1259 (gleiche 2 pre-existing Pillow-Failures).

[2026-04-26] FIX: /chat-Response provider+model Signatur — alle Code-Pfade liefern jetzt die echten Werte | src/web_server.py + tests/run_tests.py + frontend/src/components/FIXED_CHAT_RESPONSE_MODEL_SIGNATURE_2026-04-26.md | Frontend hatte BACKEND_TODO re-eskaliert, weil unter Chat-Antworten weiterhin nicht die tatsächlich verwendete Provider/Model-Combo angezeigt wurde. Ursache: Der vorherige Fix hatte nur den Happy-Path in `process_single_message` (Zeile 12216-12217) gepatcht, aber zwei Delegationspfade nicht: (A) `execute_delegation` (Zeile 2260) gab nur `provider_display`/`model_display` zurück. (B) `/api/subagent_confirm` Confirmed-Path (Zeile 12313) reichte nur `provider_display`/`model_display` an die Response weiter. Beides ergänzt um die neuen Keys `provider` + `model`, die exakt die `/models`-Endpoint-Strings spiegeln (PROVIDER_DISPLAY + MODEL_DISPLAY). Live verifiziert: Switch zwischen Sonnet 4.6 und Haiku 4.5 spiegelt sich jetzt korrekt in der Response, model-String matched 1:1 den `/models[].name`-Catalog. 12 neue Tests (Statisch: alle drei Pfade enthalten provider+model. Live: tatsächlicher Modellwechsel via /select_model spiegelt sich in /chat-Response — schützt gegen Stale-Default-Bug). Suite 1251/1253 grün (2 preexistente Pillow-Image-Fails unbezogen). BACKEND_TODO_CHAT_RESPONSE_MODEL_SIGNATURE.md → FIXED_CHAT_RESPONSE_MODEL_SIGNATURE_2026-04-26.md.

[2026-04-26] FEATURE: Modell-Katalog-Refresh — Opus 4.7 + GPT-5.x-Familie + Gemini 3.1 Flash-Lite | config/models.json + src/web_server.py + tests/run_tests.py | Audit aller LLM-Provider gegen aktuelle April-2026-Releases. Neu in models.json: (A) Anthropic claude-opus-4-7 (released 16. April 2026, live verifiziert mit konfiguriertem API-Key, antwortet "Bestätigt"). (B) OpenAI komplette GPT-5.x-Familie hinzugefügt: gpt-5.5, gpt-5.5-pro, gpt-5.4, gpt-5.4-pro, gpt-5.4-mini, gpt-5.4-nano, gpt-5.3-codex, gpt-5, gpt-5-mini, gpt-5-nano, o3-pro. Alte gpt-4o/o1 als (legacy) markiert. Quota-Status (HTTP 429 auf alle OpenAI-Modelle) bleibt — Auflistung aktualisiert das Display, Billing-Bug bleibt offen. (C) Gemini 3.1-flash-lite-preview ergänzt; 3-pro-preview ist offiziell deprecated, bleibt aber für Rückwärtskompatibilität gelistet. (D) Mistral *-latest-Aliase tracken bereits Mistral Large 3 / Small 4 / Magistral 1.2 — keine ID-Änderung nötig. (E) Perplexity & DeepSeek: nichts neu seit V4/Sonar-Lineup. (F) Ollama: Empfehlung Qwen 3.5 9B + DeepSeek R1 14B nicht hinzugefügt, weil Modelle erst per `ollama pull` gezogen werden müssen — sonst leerer Dropdown-Eintrag. MODEL_DISPLAY um neue IDs erweitert (Ausgabe via /chat-Response provider+model). 21 neue Grep+JSON-Tests (web_server.py + models.json), Suite 1239/1241 (2 pre-existing Pillow-Image-Tests). Plus: Junk-Konversation-Cleanup-Code (`_is_junk_conversation`, `_JUNK_TEST_RE`, `_meaningful_length`, POST /api/conversations/cleanup mit dry_run-Mode + .deleted_<ISO>-Soft-Move) der vor dem Session-Restart uncommitted im Working Tree lag — die zugehörigen Tests waren bereits grün, nur die Source noch nicht persistiert. Mit-committed.

[2026-04-26] FEATURE: WhatsApp-Group-Chat-Reply via Cmd+F-Suche + Frontend-TODO fuer Konversations-Aggregation | src/web_server.py + tests/run_tests.py + frontend/src/components/FRONTEND_TODO_WHATSAPP_CONVERSATION_AGGREGATION.md | User-Bug: Klick auf eine Familia-Gruppen-Nachricht + "Mit Agent antworten" -> Backend versuchte stur Phone-Lookup fuer "Familia" und scheiterte (macOS Contacts kennt keine Gruppen). Loesung: neuer Helper _open_whatsapp_with_chat_search(chat_name, message) — aktiviert WhatsApp.app, triggert Cmd+F (Chat-Suche), tippt den Chat-Namen aus source_conversation_id, sendet Return zum Auswaehlen, paste'd die Nachricht via Cmd+V (sendet NICHT — User reviewt + Return). Wird im CREATE_WHATSAPP-Parser als Fallback aufgerufen, wenn wa_phone is None UND src_type == 'whatsapp' UND src_conv gesetzt ist. Funktioniert sowohl fuer Gruppen als auch fuer unbekannte 1:1-Kontakte. Action-Marker zeigt: "WhatsApp-Chat 'Familia' via Suche geoeffnet, Nachricht eingefuegt — bitte pruefen und mit Return senden." Plus: Frontend-TODO fuer Lovable, das WhatsApp-Inbox-UX an WhatsApps native App anpasst — eine Zeile pro Konversation (gruppiert per conversation_id) statt N Zeilen pro Einzelnachricht. 11 neue Grep-Tests fuer Helper + Wiring. Pre-existing Image-Test-Failures (Pillow-Tests scheitern wenn flask in Homebrew-Python fehlt) sind NICHT betroffen.

[2026-04-26] FIX (P0 cont'd): Phantom-Duplicate blockierte eindeutigen Renata-Match | src/web_server.py + tests/run_tests.py | Nach dem empty-cname-Fix wurde Renata trotzdem nicht mehr gefunden. Ursache: macOS Contacts hatte ZWEI 'Renata Argolo'-Eintraege — einen mit Phone (+5521999485905, die richtige), einen Phantom-Duplikat ohne Phone. Mein Mehrdeutigkeits-Schutz `len(same_name) > 1 -> ambiguous` war zu konservativ und blockierte den eindeutigen Match. Fix: same_name_with_phone-Filter — wenn nur EINER der gleichnamigen Eintraege eine Phone hat, ist die Auswahl trotzdem eindeutig (Phantom-Duplikate ohne Phone sind aus iCloud-Sync-Artefakten haeufig). Mehrere mit Phone bleiben als echte Mehrdeutigkeit erkannt. Plus: Clipboard-Fallback oeffnet WhatsApp NICHT mehr automatisch — vorher zeigte 'open -a WhatsApp' den letzten aktiven Chat, der zufaellig der falsche +47-Bank-Norwegian-Chat aus einem alten Auto-Send war. Saubereres UX: User sieht klare Meldung im Chat-UI, kopiert die Nachricht aus dem Clipboard und oeffnet WhatsApp/Chat selbst. Live-verified: Renata Argolo -> +5521999485905 (exakt wie in Contacts.app), Renata-only -> Clipboard mit 7-mehrdeutige-Auflistung. Suite 1181/1181.

[2026-04-26] FIX (P0): WhatsApp-Renata-Bug — empty-cname matched alles, plus Mehrdeutigkeits-Schutz | src/web_server.py + tests/run_tests.py | Root-Cause war NICHT macOS Contacts (wie zuerst angenommen), sondern ein contact-matching-Bug in Step 2 von send_whatsapp_draft: `if cname in to_lower` ist immer True wenn cname=="" — und der signicat-Email-Watcher hatte 13 Eintraege mit name=null aus Mail-Signaturen (z.B. 'tst@banknorwegian.no' mit phone +47 950 78 047). Mein fruheres "Renata Argolo"-Lookup matchte den ersten name=null-Eintrag mit phone und kopierte deren norwegische Nummer in jeden WhatsApp-Aufruf. Fix: `cname and (to_lower in cname or cname in to_lower)` — leere/null Namen matchen nicht mehr. Plus: Step 3 (macOS Contacts) holt jetzt ALLE Eintraege (auch ohne Phone) fuer korrekte Mehrdeutigkeits-Erkennung — wenn 2 Renata Argolo im Adressbuch sind (eine mit Nummer, eine ohne), kann nicht gewaehlt werden -> Clipboard-Fallback. send_whatsapp_draft returnt jetzt 3-tuple (to, phone, hint); bei Mehrdeutigkeit landet im Action-Marker eine klare Erklaerung statt 'Aktion ausgefuehrt' (User sieht: 'KEIN AUTO-VERSAND - <reason>. Nachricht in Zwischenablage'). 10 neue Tests, Suite 1179/1179 — alle Tests gruen, inkl. der pre-existing Live-Tests.

[2026-04-25] FIX + FEATURE: 4 weitere BACKEND_TODOs (Reply-Session-Bug, Chat-Signatur, Frontend-TODOs CRUD, Renata-Wrong-Number) | src/web_server.py + tests/run_tests.py | (A) WHATSAPP_CONTACT_DISAMBIGUATION (akut): User klickte "Reply with Agent" auf Renata Argolos WhatsApp, der Agent generierte CREATE_WHATSAPP{to:Renata}, send_whatsapp_draft fragte macOS Contacts mit "name contains 'Renata'" → 8 Treffer, item 1 = Renata Rufino, falsche Nummer. Root-Cause: api_agent_sessions hat source_message_id NUR in initial_messages-Text gepackt, NICHT in Session-State. Fix: get_session(session_id) wird jetzt direkt im Session-Create gerufen, source_message_id + source_conversation_id + source_type + source_kind + source_contact_name in state geschrieben. CREATE_WHATSAPP-Parser ueberschreibt to-Halluzination ('Renata' → 'Renata Argolo' aus conversation_id) und verwirft halluzinierte phone-Werte. send_whatsapp_draft holt jetzt ALLE AppleScript-Treffer (Name+Phone) und entscheidet in Python: exact match > eindeutiger Single-Match > Mehrdeutigkeit -> Clipboard-Fallback. (B) REPLY_SESSION_CONTEXT (P1, Lovable-TODO): Konversations-Kontext landet jetzt zusaetzlich als kontext_item im Session-State, sodass der LLM beim ersten /chat-Aufruf den Thread sieht (vorher war er nur in initial_messages der Response). auto_search wird in Reply-Sessions geskippt — vorher hat keyword-basierte Search bei Folge-Messages irrelevante Email-Files in den Kontext injectet (User-Bug-Demo: Renata-Reply zeigte plotzlich PayPal/ELSTER/Post-Mails). (C) CHAT_RESPONSE_MODEL_SIGNATURE: /chat-Response liefert jetzt 'provider' + 'model' (1:1 zu /models-Endpoint), Frontend kann unter jeder Antwort die wirklich genutzte LLM-Konfig anzeigen. (D) FRONTEND_TODOS_CRUD: Neue Endpoints GET/PATCH/DELETE /api/frontend-todos[/<slug>] — list/edit/soft-delete der BACKEND_TODO_*.md im Frontend-Repo. Slug-Pattern [A-Z0-9_]+ verhindert Pfad-Traversal. DELETE?reason=fixed -> FIXED_<slug>_<date>.md, sonst .removed_<ts>. Eat-my-own-dogfood: die 3 neuen TODOs hat der Backend selbst per DELETE-API als FIXED markiert. Frontend-Repo: 3 BACKEND_TODOs -> FIXED, commit 3968bf5. 9 neue Grep-Tests, Suite 1109/1115 (6 pre-existing live-Test-Fails — /api/messages timeout, canva, alle umgebungs-bedingt).

[2026-04-25] FEATURE: Beta/Stable App-Setup — zwei parallele Apps mit getrennten Servern, Stable als Fallback | src/web_server.py + src/dashboard_window.py + scripts/promote-to-stable.sh + ~/AssistantDev-Stable/ + /Applications/AssistantDev {Beta,Stable}.app | Idee: Beta ist die laufende Entwicklungsumgebung, Stable ein eingefrorener Snapshot — wenn Beta zerschossen ist, klickst du Stable im Dock und arbeitest dort weiter, bis Beta repariert ist. (A) Port-Flex: web_server.py + dashboard_window.py respektieren ENV ASSISTANTDEV_PORT (Default 8080). Beta auf 8080, Stable auf 8090. (B) Stable-Snapshot: rsync von Beta src/ + frontend/dist/ + scripts/ + config/ + resources/ + chrome_extensions/ nach ~/AssistantDev-Stable/. Datalake (iCloud) + API-Keys (config/) bleiben SHARED — Stable hat sofort die gleichen Mails/Konversationen/Provider. (C) App-Bundles: AssistantDev Beta.app (ASSISTANT_DIR=$HOME/AssistantDev, kein Port-Override) + AssistantDev Stable.app (ASSISTANT_DIR=$HOME/AssistantDev-Stable, ASSISTANTDEV_PORT=8090). Bundle-Identifier .app.beta vs .app.stable. CFBundleDisplayName "AssistantDev Beta" / "AssistantDev Stable". (D) Promote-Script: scripts/promote-to-stable.sh stoppt Stable-Server, snapshot't Stable-Source nach backups/promote_<ts>/, rsync't Beta-Code drauf, startet Stable wieder, Healthcheck. (E) Altes /Applications/AssistantDev.app -> backups/. Icon-Tinting fuer Stable noch offen (braucht ImageMagick); aktuell unterscheiden sich die zwei Apps nur ueber den Display-Namen unter dem Icon. Beide Server live verifiziert (Beta :8080 HTTP 200, Stable :8090 HTTP 200, getrennte Python-Prozesse).

[2026-04-25] FIX: 4 BACKEND_TODOs 2026-04-25 — WhatsApp-Placeholder-Nummer, Eigene Chat-Messages, Conversation-Cleanup | src/web_server.py + tests/run_tests.py | (A) WHATSAPP_PLACEHOLDER_NUMBER + WHATSAPP_GIF_SEND (selber Root-Cause): LLM halluzinierte +490000000000 in `[CREATE_WHATSAPP:{phone:...}]`, send_whatsapp_draft uebergab das ungeprueft an `whatsapp://send?phone=...`. Neuer Helper `_is_real_phone()` prueft Mindestlaenge 8 Ziffern + keine 6+ Nullen am Stueck. Spec-Phone, Contacts-Lookup-Phones und macOS-AppleScript-Phone werden alle gegen den Helper validiert; bei Fehlschlag faellt send_whatsapp_draft auf Clipboard-Fallback (oeffnet WhatsApp ohne whatsapp://-URL). (B) CHAT_THREAD_OWN_MESSAGES: WhatsApp-Parser skippt 'Ich:'-Lines nicht mehr (vorher continue, jetzt is_own=True). _msg_is_sent_by_user respektiert is_own. /api/messages-Default-direction fuer chat-Sources (whatsapp/imessage/kchat) auf 'received' gesetzt — Inbox bleibt sauber. Hash-Sender-Fallback (CN+ptc8GIA...:) ersetzt sender_name mit `contact or "WhatsApp"`. (C) CONVERSATION_CLEANUP: /api/conversations liest jetzt VOLLES File statt 2KB-Head, message_count + neuer user_message_count + created_at sind reliable. Neuer DELETE /api/conversations/<id> mit Soft-Delete (.deleted_<unix_ts>-Suffix, CLAUDE.md-konform). Frontend-Repo: 4 BACKEND_TODOs -> FIXED, commit f31b02d. 16 neue Grep-Tests, Suite 1151/1152 (1 pre-existing Canva-Fail).

[2026-04-25] FEATURE: Local-Mode-Bypass — keine Apple-ID-Pflicht beim nativen App-Start | src/dashboard_window.py + frontend(separates Repo): src/lib/api.ts + src/components/auth/{AuthProvider,ProtectedRoute}.tsx | (A) dashboard_window.py: ASSISTANTDEV_WEBVIEW_DEBUG-Default von "1" auf "0" geflippt — die DevTools-Konsole oeffnet sich nicht mehr automatisch beim App-Start. Aktivieren via ENV oder Rechtsklick-Inspector unveraendert. (B) Frontend: Bei window.location.hostname in (localhost|127.0.0.1|::1) wird der Supabase-Init komplett uebersprungen und ProtectedRoute laesst die Route ohne Login durch. api.ts schaltet auf same-origin BASE_URL ('') statt api.bios.love — Backend gibt localhost via Host-Header sowieso einen Free-Pass. Sicherheitsmodell unveraendert: Lovable-Preview + Cloudflare-Tunnel api.bios.love brauchen weiter Apple-Sign-In + Allowlist. Frontend-Commit af09189 nach Rebase auf Lovables 18 zwischenzeitlich gemergte Commits.

[2026-04-25] FEATURE: DeepSeek + Ollama als neue Provider — V4-Pro/V4-Flash (Cloud) + R1:8B (lokal, Mac M4) | src/web_server.py + config/models.json (datalake) + tests/run_tests.py | Zwei neue Provider im Adapter-Pattern. (A) `deepseek` (Cloud): OpenAI-kompatible API gegen api.deepseek.com/v1, Modelle deepseek-v4-pro + deepseek-v4-flash (Reasoning, max_tokens=8000 weil V4 viel Platz fuer internen Reasoning-Block braucht — bei zu kleinem Limit kommt content='' zurueck). (B) `ollama` (lokal): Ollama-Daemon auf 127.0.0.1:11434 (homebrew install ollama, brew services start ollama), pull deepseek-r1:8b (5.2 GB, Llama-3.1-8B-Distill, R1-Reasoning, Apple-Silicon-Metal-Acceleration). Beide Adapter wiederverwenden den OpenAI-SDK gegen unterschiedliche base_urls — call_deepseek + call_ollama je in beiden duplizierten Adapter-Bloecken (713 + 2130) eingetragen. VALID_PROVIDERS um 'deepseek' + 'ollama' erweitert, PROVIDER_DISPLAY um 'DeepSeek' + 'Ollama (lokal)', MODEL_DISPLAY um deepseek-v4-pro/v4-flash/chat/reasoner/r1:8b/r1:14b. models.json um beide Provider-Bloecke ergaenzt (DeepSeek-API-Key + base_url localhost:11434). End-to-end verifiziert: V4-Flash → "Bestaetigt", R1:8B → "Bestätigt". 19 neue Grep+JSON-Tests, Suite 1133/1135 gruen (2 pre-existing Failures: canva-Token + /api/messages-Live-Hang).

[2026-04-25] FIX: WhatsApp + iMessage Conversation-Grouping clientseitig (Inbox-Flooding behoben) | frontend/src/pages/Messages.tsx | Globaler /api/messages-Endpoint liefert WhatsApp/iMessage flach (eine Zeile pro Einzel-Message), wodurch z.B. 500 WhatsApp-Messages auf nur 39 Konversationen die Inbox überfluten. Frontend gruppiert jetzt im conversations-useMemo pro (channel + conversation_id) — nur die neueste Message als Sichtbar, unread summiert. E-Mail-Spalten bleiben unverändert message-level. Build grün, tsc --noEmit grün. Vgl. FIXED_WHATSAPP_CONVERSATION_GROUPING.md (Frontend-Anteil war noch offen).

[2026-04-25] CHORE: 2 offene FRONTEND_TODOs → FIXED, 1 stale Inline-TODO(API) entschaerft | docs/lovable/FIXED_RESPONSE_SHAPES.md + docs/lovable/FIXED_UI_FEATURE_BATCH.md + frontend/src/components/admin/SystemPromptEditor.tsx | (A) FRONTEND_TODO_RESPONSE_SHAPES.md → FIXED_RESPONSE_SHAPES.md: alle 5 Pages (AdminHealth/Permissions/Changelog/Messages/Memory) wurden bereits an die neuen Backend-Shapes angepasst (verifiziert per grep + tsc grün). Original-Briefing als "Original-Briefing"-Sektion behalten, Erledigung im Header dokumentiert. (B) FRONTEND_TODO_UI_FEATURE_BATCH.md → FIXED_UI_FEATURE_BATCH.md: alle 11 UI-Features waren bereits im Lovable-Commit 1d17f63 (2026-04-22) umgesetzt — Per-Column Search/Sort, Grid-Layout, Density-Sweep, Pinned Messages-Tab. (C) SystemPromptEditor.tsx: TODO(API)-Marker entfernt — der Workaround zum Nachziehen der vollen description aus /api/system_prompt ist seit Backend-Update 2026-04-24 ein No-Op (description aus /agents kommt jetzt ungekuerzt), Code bleibt als defensiver Fallback fuer Edge-Cases. tsc --noEmit gruen.

[2026-04-24] SECURITY: legacy_shared_token_enabled → false (Shared-Bearer-Token extern deaktiviert) + 3 neue BACKEND_TODOs umgesetzt | config/supabase_auth.json + src/web_server.py | (A) Legacy-Shared-Token-Pfad extern abgeschaltet. Jeder extern angekommene Request braucht ab jetzt einen gueltigen Supabase-JWT; alter Token → 401 via api.bios.love. Rollback: flag zurueck auf true, redeploy (< 30s). (B) BACKEND_TODO_AGENTS_FULL_DESCRIPTION: /agents liefert description jetzt vollstaendig (User-Teil vor SYSTEM-CAPABILITIES-Marker), description_preview bleibt truncated fuer Tooltips — verhindert Datenverlust beim Save. Neuer Detail-Endpoint GET /agents/<name> fuer Single-Agent-Fetch inkl. subagents + parent-Ref. (C) BACKEND_TODO_MEMORY_AND_CONVERSATIONS: /api/memory/list/<agent> klassifiziert jetzt kind ∈ email|webclip|doc|conversation|contacts|image|memory (via Filename-Pattern, Email erkennt _IN_/_OUT_ mitten im Namen sowie email_-Prefix). Default ?include=conversations=true, scant zusaetzlich konversation_*.txt + contacts.json im Agent-Root. /api/conversations?agent=X liefert jetzt {id, agent, title, preview, updated_at, updatedAt, last_message_at, message_count, file, modified, size} — Aliase fuer Frontend-Flex. /api/conversations/<id>/messages liefert {messages:[{id, role, content, at}]} mit mtime-abgeleitetem at. (D) BACKEND_TODO_EMAIL_THREAD_RECONSTRUCTION: _email_thread_members aggregiert jetzt ueber 3 Pfade: gleicher conversation_id + gleicher normalisierter Subject-Key (richtungsunabhaengig) + In-Reply-To/Message-ID Header-Walk (transitiv). _normalize_subject strippt Re:/Fwd:/AW:-Prefixe wiederholt. Live-Test: Thread von "Your invoice from Apple." nun 12 Messages statt 1. Sent-Mail-Indexing bleibt offene Limitation (sent-Folder wird nicht gelesen). 3 BACKEND_TODOs → FIXED_*_2026-04-24.md im Frontend-Repo. 18 neue Live+Grep-Tests, Suite 1116/1117 gruen (1 pre-existing Canva-Fail).

[2026-04-23] SECURITY: Auth-Refactor — Supabase-JWT-Verifikation + per-User-Identity parallel zum Shared-Token (Backwards-compat-Phase) | src/web_server.py + config/supabase_auth.json (gitignored) + tests/run_tests.py | Backend akzeptiert jetzt Supabase-Session-JWTs (RS256/ES256) via JWKS (https://oplpeiaqsqrjtpbpvidq.supabase.co/auth/v1/.well-known/jwks.json, lazy cached via PyJWKClient). Pro Request: aud=authenticated + exp gecheckt, claims.email gegen lokale Allowlist (moritz.cremer@me.com, m.cremer911@gmx.de) verglichen — 403 bei Non-Allowlist, 401 bei invalid_token. Erfolg → g.user_id=sub, g.user_email=email, g.auth_source='supabase_jwt' fuer spaetere per-User-Authz. Shared-Token-Pfad laeuft parallel hinter Flag legacy_shared_token_enabled (Default true) in config/supabase_auth.json — damit Lovable in Ruhe migrieren kann. Auth-Prioritaet: JWT > Shared-Token. /api/health + /api/health-status bleiben unauthed (Monitor-Probes). Dependency pyjwt[crypto] 2.12.1 via pip installiert. Frontend-Bonus: AdminHealth.tsx Restart-Button von UnavailableAction auf echten Button mit useMutation umgestellt (Endpoint /api/health/<svc>/restart), stale TODO(API)-Kommentar entfernt. 13 neue Live+Grep-Tests, Suite 1098/1099.

[2026-04-23] SECURITY: Shared Bearer Token rotiert — alter Wert (im Frontend-Bundle + Git-History exponiert) invalidiert | config/api_auth.json | Neuer 43-char urlsafe Token via secrets.token_urlsafe(32). Auth-Middleware lädt api_auth.json bei jedem externen Request frisch, daher gilt die Rotation mit Deploy. Verifikation: Alter Token → 401 via api.bios.love, neuer → 200. Repo-Hygiene: Klartext-Token in FIXED_API_BASE_URL.md + (vormals) BACKEND_TODO_TOKEN_ROTATION.md durch <rotated-2026-04-23> ersetzt; git grep des alten Tokens ist leer. Git-History bleibt exponiert — daher zwingende Invalidierung. User-Folge: Lovable Build Secrets VITE_API_TOKEN updaten + republishen. Langfrist-Refactor: BACKEND_TODO_AUTH_REFACTOR.md (Supabase-JWT-Forwarding) bleibt offen als separater Task.

[2026-04-23] FEATURE: API-Gaps-Batch 2026-04-22 — 7 Gaps umgesetzt (Custom Sources CRUD, Mark-read-Aliase, Changelog-Parser, Health+Services, Memory-Aggregat, Docs-Dedupe) | src/web_server.py + tests/run_tests.py | (§1) Custom Sources komplette CRUD: GET/POST/PUT/DELETE /custom_sources, /custom_sources/:id/sync, /custom_sources/validate. Storage in config/custom_sources.json (datalake), Validator fuer folder/rss/url/notion/gist mit URL-Kind-Detection + Sample-Preview. (§2) /api/messages/mark-read akzeptiert jetzt message_id ODER id ODER conversation_id + mode='conversation' → markiert alle Messages der Conv. 404 statt 400 bei 'no matches'. (§3) Changelog-Parser extended: parst one-liner `[YYYY-MM-DD] TYPE: title | file | reason` am Kopf + Legacy `## YYYY-MM-DD`/`### Feature:` im Body + sortiert nach date desc. Liefert jetzt 21 statt 1 Eintrag. (§5) /api/health ergaenzt um cloudflared, kchat_watcher, whatsapp_import, contact_watchdog, calendar_export (jetzt 8 Services). Restart-Endpoint _KNOWN_SERVICES erweitert. (§6) /api/memory/all Sammel-Endpoint: ?flat=1 oder dict{agent:files[]}, ?include=conversations. (§7) /api/memory/list/<agent>?include=conversations ergaenzt konversation_*.txt mit kind='conversation' (vs kind='memory'). (§8) /api/docs/<slug> und /api/docs/read/<slug> liefern beide identisches Shape {slug,name,title,content,markdown} — kein Feldname-Mismatch mehr. 40 neue Live+Grep-Tests, Suite 1085/1086.

[2026-04-22] FEATURE: /api/system_prompt strukturiert + /api/health/<service>/restart | src/web_server.py + tests/run_tests.py | (1) GET /api/system_prompt/<agent>[/<sub>] liefert jetzt zusätzlich user/generated/sections[] (vorher nur prompt). Split am Marker '--- SYSTEM CAPABILITIES (AUTO-GENERATED - DO NOT EDIT BELOW) ---', Sections aus '## Heading'-Zeilen im generated-Teil. Frontend-Editor kann User-Teil editieren + Capabilities read-only als separate Panels zeigen (BACKEND_TODO_SYSTEM_PROMPT_SECTIONS). (2) POST /api/health/<service>/restart {op:'restart'|'stop'} implementiert: launchctl kickstart -k für web_server/email_watcher (haben LaunchAgent), pkill+Popen für web_clipper. 400 bei bad op, 404 bei unbekanntem Service. Lovable-Inline-Marker (AdminHealth.tsx:190) erledigt. 13 neue Live+Grep-Tests, Suite 1058/1059.

[2026-04-22] FEATURE: Lovable Messages-Batch 2026-04-22 — 11 UI-Feature-TODOs via Lovable+Claude umgesetzt (Per-Column Search/Sort, Grid-Layout, Density, Pinned Messages-Tab) | frontend/src/pages/Messages.tsx + frontend/src/components/layout/{Shell,PageHeader}.tsx + frontend/src/pages/{Dashboard,Chat}.tsx | Lovable-Commit 1d17f63 "Density-Sweep und Messages neu" hat implementiert: (1) Flex→Grid-Layout [auto-fit,minmax(220px,1fr)] → alle 7 Spalten ohne H-Scroll; (2) Per-Spalten-Suche via /api/messages/search (backend-integriert, 250ms debounce via useDebounced); (3) Per-Spalten-Sort-Dropdown (newest/oldest/unread-first, 3 Optionen); (4) Messages-Tab pinned on mount via ensureSystemTab('messages')+setPinned(TAB_MESSAGES_ID,true); (5) Density: Shell p-6→p-4/p-6, PageHeader mb-8→mb-5, Dashboard-Cards p-5→p-4 + h-11→h-9 + text-2xl→text-xl. Claude-Commit b191ac7 ergaenzt: Chat.tsx PageHeader entfernt + durch 1-Zeiler-Strip ersetzt (-70px vertikal), 11 BACKEND_TODO_*.md → FIXED_*.md umbenannt (Lovable hatte die Renames vergessen), .gitignore fuer tsconfig.tsbuildinfo. Build gruen.

[2026-04-22] FEATURE: Lovable Messages-Batch 2026-04-22 — 3 weitere BACKEND_TODOs umgesetzt (PARALLEL_SESSIONS / API_BASE_URL / UNREAD_EMAIL_ONLY) | src/web_server.py | REST-Aliase fuer Session-Lifecycle: POST /chat/cancel (alias zu /stop_queue), DELETE /session?session_id=... (Cleanup beim Tab-Close), POST /ctx (setzt/ersetzt Context-Items pro Session). supports_read-Flag in /api/messages/sources[] pro Quelle (email=true, whatsapp/imessage/kchat=false) — Frontend kann Read-Toggle + Unread-Badge conditionally rendern. API_BASE_URL war bereits umgesetzt (api.ts hardcoded + .env.production). 11 weitere TODOs (PER_COLUMN_SEARCH*7, SORT*2, DENSITY_WHITESPACE, MESSAGES_LAYOUT, PINNED_TAB) sind reine Frontend-UI-Features und delegated an Lovable via docs/lovable/FRONTEND_TODO_UI_FEATURE_BATCH.md. Suite gruen.

[2026-04-22] FEATURE: Lovable Messages-Batch 2026-04-22 — 6 neue BACKEND_TODOs umgesetzt | src/web_server.py | (1) /api/messages erweitert um ?direction=received|sent|all (Default=received fuer email), ?bucket=inbox|other (Domain-Exclude fuer @tangerina.me/@signicat.com/@trustedcarrier.net), ?group=conversation (aggregiert per conversation_id mit unread_count/last_message/participants); (2) /api/messages/search neu — scoped Suche pro Source mit <mark>-Highlighting; (3) /api/messages/<id>/thread + /api/messages/conversation/<cid> — Thread/Conversation-Detail; (4) /api/messages/<id>/reply — erzeugt AppleScript-Draft in Apple Mail (Fire-and-forget); (5) /api/contacts — Adressbuch aus E-Mail-Absendern (226 Kontakte aktuell, ?view=recent|all, ?q=search); (6) POST /api/agents/<name>/sessions — neue Agent-Session mit optionalem intent:"reply_to_message" + source_message_id (bettet Thread-Kontext als initial_messages ein). 9 neue Live-Tests, Suite 1045/1046 gruen. Frontend-Repo: 6 BACKEND_TODO umbenannt zu FIXED_*. docs/lovable/WORKFLOW.md fuer Lovable-Memory/Instructions.
